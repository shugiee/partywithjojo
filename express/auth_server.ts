...
const getAllMembersInPartyWith = (name: string): Row[] => {
    const row = db.prepare('SELECT * FROM guests WHERE party_id IN (SELECT party_id FROM guests WHERE name LIKE ?);').all(name) as Row[];
    return row;
};
...
const toggleWelcomePartyAttendanceForUser = (name: string, isEnabled: boolean) => {
    const value = isEnabled ? 1 : 0;
...
const checkbox = (guestName: string, httpTarget: string, isEnabled: boolean) => {
